...
const ReturnList = ({ transactions, onReturn, loading, returningId }) => {
  ...
  const filtered = transactions?.filter(t => {
    const matchStatus = filterStatus === 'all' ? true : filterStatus === 'returned' ? t?.returned : !t?.returned;
  ...
  return (
    <div className="bg-[var(--color-card)] rounded-[var(--radius-lg)] border border-[var(--color-border)] shadow-[var(--shadow-md)] p-4 md:p-6">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-lg font-semibold" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-foreground)' }}>Transaction Records</h2>
          <p className="text-xs" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>Manage returns and view borrow history</p>
        </div>
      </div>
      {/* Filters */}
      ...
      {filtered?.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 gap-3">
          ...
          <p className="text-sm" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>No transactions found.</p>
        </div>
      ) : (
        <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1">
          {filtered?.map(t => (
            <div
              ...
              style={{
                borderColor: t?.returned ? 'var(--color-border)' : 'rgba(232,90,79,0.25)',
                backgroundColor: t?.returned ? 'var(--color-background)' : 'rgba(232,90,79,0.04)',
              }}
            >
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <span className="font-semibold text-sm truncate" style={{ color: 'var(--color-foreground)', fontFamily: 'var(--font-heading)' }}>{t?.itemName}</span>
                  <span
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold"
                    style={{
                      backgroundColor: t?.returned ? 'rgba(34,197,94,0.12)' : 'rgba(232,90,79,0.12)',
                      color: t?.returned ? 'var(--color-success)' : 'var(--color-accent)',
                      fontFamily: 'var(--font-caption)',
                    }}
                  >
                    <Icon name={t?.returned ? 'CheckCircle' : 'Clock'} size={11} color="currentColor" strokeWidth={2.5} />
                    {t?.returned ? 'Returned' : 'Not Returned'}
                  </span>
                </div>
                <div className="flex flex-wrap gap-x-4 gap-y-1">
                  ...
                  <span className="text-xs flex items-center gap-1" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>
                    ...
                    Borrowed: {format(new Date(t.borrowedAt), 'MM/dd/yyyy hh:mm a')}
                  </span>
                  {t?.returned && t?.returnedAt && (
                    <span className="text-xs flex items-center gap-1" style={{ color: 'var(--color-muted-foreground)', fontFamily: 'var(--font-caption)' }}>
                      <Icon name="RotateCcw" size={12} color="currentColor" strokeWidth={2} />
                      Returned: {format(new Date(t.returnedAt), 'MM/dd/yyyy hh:mm a')}
                    </span>
                  )}
                </div>
              </div>
